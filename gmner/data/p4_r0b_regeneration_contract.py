...
P4_R0B_PREREGISTRATION_KIND = (
    "final_chain_oof_fold0_dry_run_preregistration"
)
P4_R0B_FORMAT_VERSION = 1
P4_R0B_ARTIFACT_IDENTITY = "FINAL_CHAIN_OOF_FOLD0_DRY_RUN"
P4_R0B_EXECUTION_FOLDS = (0,)
...
def validate_r0b_preregistration(payload: dict) -> None:
    ...
    if payload.get("status") != "AUTHORIZED_NOT_STARTED":
        raise PermissionError("Fold-0 dry-run is not in the authorized state.")
    authorization = dict(payload.get("authorization") or {})
    if authorization.get("fold0_execution") is not True:
        raise PermissionError("Fold-0 execution is not authorized.")
    locked = (
        "folds_1_9_execution",
        "b1_a1_population_training",
        "dev_execution",
        "test_execution",
    )
    ...
    if int(source.get("seed", -1)) != 42 or int(source.get("num_folds", -1)) != 10:
        raise ValueError("Fold-0 dry-run fold contract changed.")
    if source.get("official_dev_access") is not False or source.get("test_access") is not False:
        raise PermissionError("Official Dev/Test access must remain disabled.")
    ...
    if (
        ...
        raise PermissionError("Fold-0 must use only the formal M3.3A chain.")
